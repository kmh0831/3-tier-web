...
router.get('/favorites', async (req, res) => {
  ...
  try {
    connection = await mysql.createConnection(dbConfig);
    const [favorites] = await connection.query('SELECT m.* FROM favorites f JOIN movies m ON f.movie_id = m.id WHERE f.user_id = ?', [userId]);
    res.json(favorites);
  } catch (error) {
...
// 찜하기 취소 기능 추가
router.delete('/favorite/:movieId', async (req, res) => {
  ...
  try {
    ...
    res.json({ message: '찜하기 취소 완료' });
  } catch (error) {
    console.error('찜하기 취소 처리 중 오류 발생:', error);
    res.status(500).json({ error: '찜하기 취소 처리 중 오류 발생' });
  } finally {
    if (connection) await connection.end();
  }
});
